...
namespace Plugin.NewtonsoftJsonPinPointClearCycleB;
...
public class TestCase
{
    public void Execute()
    {
        JsonConvert.SerializeObject(new LocalTypeB { Y = "hello" });
        ClearCache();

        // Post-cleanup verification: BCL types should still serialize/deserialize correctly
        var dict = new Dictionary<string, int> { { "a", 1 } };
        var json = JsonConvert.SerializeObject(dict);
        var rt = JsonConvert.DeserializeObject<Dictionary<string, int>>(json);
        if (rt == null || rt["a"] != 1)
            throw new InvalidOperationException("BCL serialization broken after CycleA cleanup");
    }
