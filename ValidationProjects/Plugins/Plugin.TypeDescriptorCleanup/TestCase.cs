...
namespace Plugin.TypeDescriptorCleanup;
...
public class TestCase
{
    public void Execute()
    {
        var provider = new LocalTypeDescriptionProvider();
        TypeDescriptor.AddProvider(provider, typeof(LocalType));

        // Exercise the type through TypeDescriptor to populate secondary caches.
        TypeDescriptor.GetProperties(typeof(LocalType));
        TypeDescriptor.GetConverter(typeof(LocalType));

        // Public API: remove the explicitly-added provider.
        TypeDescriptor.RemoveProvider(provider, typeof(LocalType));
    ...
    public static void ClearTypeDescriptorCaches()
    {
        ...
        // Prune s_providerTable entries that reference collectible assembly types.
        Prune(providerTable);
